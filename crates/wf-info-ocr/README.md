...
`wf-info-ocr` reads item names from cropped Warframe UI images.
...
```sh
cargo run -p wf-info-ocr --example ocr_inventory
cargo run -p wf-info-ocr --example ocr_reward_screen
```
